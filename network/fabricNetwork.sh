...
function replacePrivateKey() {
  ...
  cd crypto-config/peerOrganizations/microsoft.certification-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/microsoft_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
...
function generateChannelArtifacts() {
  ...
  set -x
  configtxgen -profile OrdererGenesis -channelID microsoft-sys-channel -outputBlock ./channel-artifacts/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile CertificationChannel -outputAnchorPeersUpdate ./channel-artifacts/microsoftMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg microsoftMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for microsoft..."
    exit 1
  fi
  echo
